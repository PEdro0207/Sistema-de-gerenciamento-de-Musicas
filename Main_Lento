...
def measure_time(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        print(f"[LENTO: {func.__name__}] Executado em {elapsed:.4f}s")
        return result
    return wrapper
...
def bubble_sort(items, sort_key):
    """
    BubbleSort (O(n²)) — compara e troca elementos repetidamente.
...
def get_db():
    ...
    Utiliza Flask's 'g' para armazenar a conexão por request
...
def fetch_songs_by_genre(genre_keyword, limit=100):
    ...
    try:
        ...
        print(f" Ordenação por BubbleSort levou {elapsed:.4f}s para {len(songs_list)} itens")
...
# rota de busca_ barra de pesquisa
...
def search():
    ...
    try:
        ...
        # Parâmetro opcional para forçar varredura completa 
        ...
        if full_scan:
            ...
            print(f"Scan completo, carregadas {total_loaded} linhas em {scan_time:.4f}s")
        else:
            # Pré-filtra candidatos por SQL 
            print(" Usando pré-filtragem SQL para reduzir candidatos")
            cursor.execute("""
                SELECT track_name, artist_name, formatted_duration, normalized_track, normalized_artist
                FROM musicas
            ...
            rows = cursor.fetchall()
            print(f"SQL retornou {len(rows)} candidatos")
        ...
        print(f"BubbleSort levou {sort_elapsed:.4f}s para ordenar {len(results)} itens (O(n²))")
        ...
        print(f" BUSCA TOTAL lento: {total_time:.4f}s | {len(final_results)} resultados\n")
...
def playlists():
    ...
    try:
        db = get_db()
        cursor = db.cursor()
        # Busca sem ORDER BY e ordena em memória via BubbleSort para lentidão
        ...
        # Ordena em memória por created_at 
        def playlist_key(p):
...
def playlist_detail(playlist_id):
    try:
        ...
        try:
            ...
            # Ordena em memória por nome para lentidão
